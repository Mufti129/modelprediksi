...
if check_password():
    ...
    with sum_col1:
        ...
        if 'Mismatch_OLS' in df_filtered.columns:
            df_match_ols = df_filtered[df_filtered['Mismatch_OLS'] == 'Match']
            df_mism_ols = df_filtered[df_filtered['Mismatch_OLS'] != 'Match']
            
            ols_match = len(df_match_ols)
            ols_mismatch = len(df_mism_ols)
            
            pct_match = (ols_match / total_data * 100) if total_data > 0 else 0
            pct_mism = (ols_mismatch / total_data * 100) if total_data > 0 else 0
            
            st.success(f"✅ **Match:** {ols_match} Cabang ({pct_match:.1f}%)")
            st.error(f"❌ **Mismatch:** {ols_mismatch} Cabang ({pct_mism:.1f}%)")
            
            filter_ols = st.radio(
                "Filter Tampilan OLS:",
                ["Semua OLS", "Hanya Match", "Hanya Mismatch"],
                key="filter_ols_key",
                horizontal=True
            )
            
            if filter_ols == "Hanya Match" or filter_ols == "Semua OLS":
                if not df_match_ols.empty:
                    st.markdown("**Rincian Pola Match (Prediksi ➔ Aktual):**")
                    match_counts_ols = {}
                    for _, r in df_match_ols.iterrows():
                        kombinasi = f"{r.get('Kategori_Prediksi_OLS', '-')} ➔ {r.get('Kategori_Omzet_Actual', '-')}"
                        match_counts_ols[kombinasi] = match_counts_ols.get(kombinasi, 0) + 1
                    for key, val in sorted(match_counts_ols.items()):
                        pct = (val / total_data * 100) if total_data > 0 else 0
                        st.write(f"• {key}: **{val} Cabang ({pct:.1f}%)**")
            
            if filter_ols == "Hanya Mismatch" or filter_ols == "Semua OLS":
                if not df_mism_ols.empty:
                    st.markdown("**Rincian Pola Mismatch (Prediksi ➔ Aktual):**")
                    counts_ols = {}
    ...
    with sum_col2:
        ...
        if 'Mismatch_RF' in df_filtered.columns:
            df_match_rf = df_filtered[df_filtered['Mismatch_RF'] == 'Match']
            df_mism_rf = df_filtered[df_filtered['Mismatch_RF'] != 'Match']
            
            rf_match = len(df_match_rf)
            rf_mismatch = len(df_mism_rf)
            
            pct_match = (rf_match / total_data * 100) if total_data > 0 else 0
            pct_mism = (rf_mismatch / total_data * 100) if total_data > 0 else 0
            
            st.success(f"✅ **Match:** {rf_match} Cabang ({pct_match:.1f}%)")
            st.error(f"❌ **Mismatch:** {rf_mismatch} Cabang ({pct_mism:.1f}%)")
            
            filter_rf = st.radio(
                "Filter Tampilan RF:",
                ["Semua RF", "Hanya Match", "Hanya Mismatch"],
                key="filter_rf_key",
                horizontal=True
            )
            
            if filter_rf == "Hanya Match" or filter_rf == "Semua RF":
                if not df_match_rf.empty:
                    st.markdown("**Rincian Pola Match (Prediksi ➔ Aktual):**")
                    match_counts_rf = {}
                    for _, r in df_match_rf.iterrows():
                        kombinasi = f"{r.get('Kategori_Prediksi_RF', '-')} ➔ {r.get('Kategori_Omzet_Actual', '-')}"
                        match_counts_rf[kombinasi] = match_counts_rf.get(kombinasi, 0) + 1
                    for key, val in sorted(match_counts_rf.items()):
                        pct = (val / total_data * 100) if total_data > 0 else 0
                        st.write(f"• {key}: **{val} Cabang ({pct:.1f}%)**")
            
            if filter_rf == "Hanya Mismatch" or filter_rf == "Semua RF":
    ...
    with sum_col3:
        ...
        if 'Mismatch_GWR' in df_filtered.columns:
            df_match_gwr = df_filtered[df_filtered['Mismatch_GWR'] == 'Match']
            df_mism_gwr = df_filtered[df_filtered['Mismatch_GWR'] != 'Match']
            
            gwr_match = len(df_match_gwr)
            gwr_mismatch = len(df_mism_gwr)
            
            pct_match = (gwr_match / total_data * 100) if total_data > 0 else 0
            pct_mism = (gwr_mismatch / total_data * 100) if total_data > 0 else 0
            
            st.success(f"✅ **Match:** {gwr_match} Cabang ({pct_match:.1f}%)")
            st.error(f"❌ **Mismatch:** {gwr_mismatch} Cabang ({pct_mism:.1f}%)")
            
            filter_gwr = st.radio(
                "Filter Tampilan GWR:",
                ["Semua GWR", "Hanya Match", "Hanya Mismatch"],
                key="filter_gwr_key",
                horizontal=True
            )
            
            if filter_gwr == "Hanya Match" or filter_gwr == "Semua GWR":
                if not df_match_gwr.empty:
                    st.markdown("**Rincian Pola Match (Prediksi ➔ Aktual):**")
                    match_counts_gwr = {}
                    for _, r in df_match_gwr.iterrows():
                        kombinasi = f"{r.get('Kategori_Prediksi_GWR', '-')} ➔ {r.get('Kategori_Omzet_Actual', '-')}"
                        match_counts_gwr[kombinasi] = match_counts_gwr.get(kombinasi, 0) + 1
                    for key, val in sorted(match_counts_gwr.items()):
                        pct = (val / total_data * 100) if total_data > 0 else 0
                        st.write(f"• {key}: **{val} Cabang ({pct:.1f}%)**")
            
            if filter_gwr == "Hanya Mismatch" or filter_gwr == "Semua GWR":
